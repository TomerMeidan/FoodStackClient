...
public class BranchManagerUploadQuarterlyReportsWindow {
	...
	/** Get Relevant Branch Report Info<p>
	 * 
	 * This method returns a JSON holding relevant information about what time and place the report is taking 
	 * place, for example, the method takes three global variables and updates the relevant information there
	 * is to use for the current last quarter of this year or the next one.
	 * 
	 * @return The object returned is JSONObject that contains the relevant information about the current guidelines
	 * which data should be saved for the FILE creation.
	 * @author Tomer Meidan
	 ...
	 * In this manner, it is able to update the current last quarter that passed, which branch the BM
	 * belongs to and also set some FX visuals for the gui side such as the name of the BM, year, month and quarter.
	 * The purpose of this method is to determine which current last quarter has passed to enable the BM the upload
	 * option only by the last quarter.
	 * @author Tomer Meidan
	 * 
	 * */
	public void setLabels() {
			...
			Date now = new Date();
			managerName = Message.getValue(personalInfo, "FirstName") + " "
					+ Message.getValue(personalInfo, "LastName");
			...
			else yearNameLabel.setText(now.getYear() + 1900 + "");
	 ...
	 * @author Tomer Meidan
	 * @param month - input is the current month (INT ranges between 0 to 11 depending on the current month)
	...
	/** On Back Button<p>
	 * 
	 * This method is a FX trigger that will be initiated upon clicking the back button on the current upload
	 * reports window screen. the method will initiate a DESTRUCTOR that will reset all relevant variables to
	 * the class and returns to the previous page by using the view communication controllers showWindow option.
	...
	/** On Upload Reports Button<p>
	 * This method is a defined FX trigger to be initiated upon clicking the upload
	 * reports button. this method calls in the PDFGenerator class and creates the PDF file 
	 * according to the report file that was given in the global variable as allReports.<p>
	 * @Stages
	 * There are three stages in this method, firstly the PDF class will generate a pdf file and temporarily
	 * saves it on the src directory. Next, the temp file will be transported into a bytearray object and will be deleted from the computer.
	 * And lastly, using the Base64 encoding system, the file's bytearray is turned into an encoded String object in
	 * order to send it to the server side inside a JSON object. 
	 * @param event - indicates that an event was pressed on the upload report button option.
	 * @author Tomer Meidan
	 * */
	@FXML
	void onUploadReportsButton(ActionEvent event) {
		...
		try {
			...
			byte[] bytes = getByteArrayFromFile(newFile);
	...
	/** Get Byte Array From File<p>
	 * This method takes a certain File object and turns the file into a byte array object.
	 * The method is static so it can be used by other places in the project.
	 ...
	 * (Income from all restaurants, Items per restaurant and performance of the deliveries)
	 * @author Tomer Meidan
	 * */
